...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  void _seekContinuously(bool begin, int direction) {
    _seeker?.stop();
    if (begin) {
      final d = currentMediaItem?.duration;
  ...
  /// Android compact view allows at most 3 actions — prev, play/pause, **stop**
  /// (stop is easy to miss if only in the expanded row). Next / rewind / FF stay
  /// in the expanded notification.
  static List<int> _compactNotificationIndices(List<MediaControl> controls) {
    int indexOf(MediaAction action) =>
        controls.indexWhere((c) => c.action == action);

    void addDistinct(List<int> out, int i) {
      if (i < 0 || out.length >= 3) return;
      if (!out.contains(i)) out.add(i);
    }

    final out = <int>[];
    addDistinct(out, indexOf(MediaAction.skipToPrevious));
    final pauseI = indexOf(MediaAction.pause);
    final playI = indexOf(MediaAction.play);
    if (pauseI >= 0) {
      addDistinct(out, pauseI);
    } else {
      addDistinct(out, playI);
    }
    addDistinct(out, indexOf(MediaAction.stop));
    if (out.length < 3) {
      addDistinct(out, indexOf(MediaAction.skipToNext));
    }
    if (out.length < 2) {
      addDistinct(out, indexOf(MediaAction.rewind));
      addDistinct(out, indexOf(MediaAction.fastForward));
    }
  ...
  void _broadcastState() {
    // Stop last so it is at the far end and less likely to be hit accidentally.
    ...
    playbackState.add(playbackState.nvalue!.copyWith(
      ...
      androidCompactActionIndices: _compactNotificationIndices(controls),
